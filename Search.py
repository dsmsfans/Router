...
def check_flag():
    ...
    if Brand_flag or Model_flag or Type_flag or Interface_flag or Result_flag or Diamond_mode_flag != 0:
        if Brand_flag == 1:
            ...
            option = option & data["Brand"].isin([brand])
            
        if Model_flag == 1:
            model = str(input("Input your Model: "))
            option = option & data["Model"].isin([model])
            
        if Type_flag == 1:
            what_type = str(input("Input your Type: "))
            option = option & data["Type"].isin([what_type])
        
        if Diamond_mode_flag == 1:
            diamond_mode = str(input("Input your Diamond_Mode: "))
            option = option & data["Diamond Mode"].isin([diamond_mode])
            
        if Interface_flag == 1:
            interface = str(input("Input your Interface: "))
            option = option & data["Interface"].isin([interface])
            
        if Result_flag == 1:
            result = str(input("Input your Result: "))
            option = option & data["Result"].isin([result])
